...
#include "sims_plot_cluster_tree_t.hpp"




std::map<sims_plot_cluster_tree_t::column_t,std::string> sims_plot_cluster_tree_t::column_names{
    {column_t::root,"cluster"}
};


void sims_plot_cluster_tree_t::set_graph_id_in_item(int graph_id, QStandardItem *item)
{
...
bool sims_plot_cluster_tree_t::index_has_data(const QModelIndex& idx)
{
...
quantity::quantity_t* sims_plot_cluster_tree_t::get_quantity_in_item_from_Qindex(const QModelIndex& idx)
{
...
quantity::quantity_t* sims_plot_cluster_tree_t::get_quantity_in_cluster_from_Qindex(const QModelIndex& idx)
{
    if (!index_has_data(idx))
        return nullptr;
...
void sims_plot_cluster_tree_t::set_quantity_in_QStandardItem(QStandardItem* quantity_item, quantity::quantity_t& quantity)
{
    QVariant variant_data;
...
sims_plot_cluster_tree_t::sims_plot_cluster_tree_t(QWidget *parent) :
    parent(parent),
    logger(__FILE__,"sims_plot_cluster_tree_t"),
...
void sims_plot_cluster_tree_t::set_measurement(measurements_::sims_t* new_measurement)
{
    unset_measurement();
...
QStandardItem* sims_plot_cluster_tree_t::item(quantity::quantity_t& quantity, column_t col)
{
...
//            int graph_id = sims_plotwindow->add_graph(x_axis.quantity(*measurement), quantity);
            int graph_id = sims_plotwindow->add_graph(measurement->crater, quantity);
...
QStandardItem* sims_plot_cluster_tree_t::item(cluster_t& cluster, column_t col)
{
...
bool sims_plot_cluster_tree_t::is_plot_window() const
{
...
bool sims_plot_cluster_tree_t::is_measurement() const
{
...
void sims_plot_cluster_tree_t::set_model()
{
    model->clear();
...
void sims_plot_cluster_tree_t::set_header()
{
    QStringList list;
...
void sims_plot_cluster_tree_t::unset_measurement()
{
    measurement = nullptr;
    ///remove all rows
...
void sims_plot_cluster_tree_t::set_sims_plotwindow(sims_plotwindow_t* sims_plotwindow_s)
{
    sims_plotwindow = sims_plotwindow_s;
}


void sims_plot_cluster_tree_t::update()
{
    if (!is_plot_window())
        return;
