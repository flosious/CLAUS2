...
/***************************************/
/*****  filecontents::dsims_t::column_t    ****/
/***************************************/

void filecontents::dsims_t::column_t::to_screen()
{
	cout << "unit="<<unit<<"\tcluster_name="<<cluster_name<<"\tdimension="<<dimension<<"\tdata.size()="<<data.size()<<endl;
}

/***************************************/
/*****  filecontents::dsims_t::Ipr_t       ****/
/***************************************/

// const sputter_depth_t filecontents::dsims_t::Ipr_t::sputter_depth() const
// {
// 	return sputter_depth_p;
// }
// 
// const sputter_time_t filecontents::dsims_t::Ipr_t::sputter_time() const
// {
// 	return sputter_time_p;
// }
// 
// const sputter_current_t filecontents::dsims_t::Ipr_t::sputter_current() const
// {
// 	return sputter_current_p;
// }

filecontents::dsims_t::Ipr_t::Ipr_t(sputter_current_t sputter_current_s, 
...
const std::__cxx11::string filecontents::dsims_t::Ipr_t::to_string(const string del) const
{
	stringstream out;
...
/***************************************/
/*****  filecontents::dsims_t              ****/
/***************************************/

filecontents::dsims_t::dsims_t(filenames::dsims_t& name) : sims_t(name,"\t",{"*** DATA FILES ***"}), name(name)
{
}

void filecontents::dsims_t::to_screen(string prefix)
{
...
vector<cluster_t> filecontents::dsims_t::clusters()
{
...
// 		cluster.concentration_p=concentration_s;
		if (cluster.is_set()) collected_clusters.push_back(cluster);
			...
			logger::error("filecontents::dsims_t::clusters() cluster.is_set()", false);
	}
	return collected_clusters;
}

const vector<filecontents::dsims_t::column_t> filecontents::dsims_t::columns()
{
	if (check_Ipr()) ipr_shift_correction();
	remove_corrupted_lines();
	if (cluster_names().size()<1 && !parse_units_dimensions_clusternames())
	{
		logger::debug("filecontents::dsims_t::columns(): !parse_units_dimensions_clusternames");
		return {};
	}
	
	vector<dsims_t::column_t> cols;
...
const filecontents::dsims_t::Ipr_t filecontents::dsims_t::Ipr()
{
...
const vector<std::__cxx11::string>& filecontents::dsims_t::dimensions()
{
	if (dimensions_p.size()>0) return dimensions_p;
	parse_units_dimensions_clusternames();
	return dimensions_p;
}
const vector<std::__cxx11::string>& filecontents::dsims_t::units()
{
	if (units_p.size()>0) return units_p;
	parse_units_dimensions_clusternames();
	return units_p;
}
const vector<std::__cxx11::string>& filecontents::dsims_t::cluster_names()
{
...
bool filecontents::dsims_t::parse_units_dimensions_clusternames()
{
	...
	{
// 		error=true;
		logger::debug("filecontents::dsims_t::parse_raw_header_tensor(): samples.size()!=1");
		return false;
	}
	...
	if (units_p.size()!=dimensions_p.size()) {
		logger::fatal("filecontents::dsims_t::parse_raw_header_tensor(): units.size()!=dimensions.size()");
		return false;
	}
...
bool filecontents::dsims_t::check_Ipr() 
{
	is_Ipr=false;
...
set<int> filecontents::dsims_t::corrupted_lines()
{
...
void filecontents::dsims_t::remove_corrupted_lines()
{
	...
	{
		logger::info("filecontents::dsims_t::remove_corrupted_lines()", *it);
 ...
 */
bool filecontents::dsims_t::ipr_shift_correction() 
{
	raw_data_tensor();
	vector<vector<string>>* mat = &raw_data_tensor_p.at(0);
    if (mat->size()==0) 
	{
		logger::error("filecontents::dsims_t::ipr_shift_correction(): raw_data_tensor_p.at(0).size()==0", (int)mat->size());
		return false;
	}
...
const quantity_t filecontents::dsims_t::get_quantity_from_dimension_and_clustername(const string col_dimension, const string ignore_clustername)
{
	string unit="";
	...
	if (unit=="") 
	{
		logger::error("filecontents::dsims_t::sputter_time() unit",unit);
		return {};
	}
	return quantity_t{statistics::common_vector(data_vecs),unit};
}

const map<string,string>& filecontents::dsims_t::infos_and_settings()
{
...
const tm filecontents::dsims_t::start_date_time()
{
...
const tm filecontents::dsims_t::creation_date_time()
{
...
const string filecontents::dsims_t::secondary_polarity(const string find_this)
{
	if (infos_and_settings().find(find_this)==infos_and_settings().end()) return {};
	return infos_and_settings().at(find_this);
}

const energy_t filecontents::dsims_t::sputter_energy(const string find_this)
{
	if (infos_and_settings().find(find_this)==infos_and_settings().end()) return {};
...
const element_t filecontents::dsims_t::sputter_element(const string find_this)
{
	if (infos_and_settings().find(find_this)==infos_and_settings().end()) return {};
...
const secondary_voltage_t filecontents::dsims_t::secondary_voltage(const string find_this)
{
	if (infos_and_settings().find(find_this)==infos_and_settings().end()) return {};
	double data = tools::str::str_to_double(infos_and_settings().at(find_this));
...
const rastersize_t filecontents::dsims_t::sputter_rastersize(const string find_this)
{
	if (infos_and_settings().find(find_this)==infos_and_settings().end()) return {};
	double data = tools::str::str_to_double(infos_and_settings().at(find_this));
...
const rastersize_t filecontents::dsims_t::analyzed_area(const string find_this)
{
	if (infos_and_settings().find(find_this)==infos_and_settings().end()) return {};
	double data = tools::str::str_to_double(infos_and_settings().at(find_this));
	return rastersize_t({data},{"um"});
...
const quantity_t filecontents::dsims_t::chamber_pressure(const string find_this)
{
	if (infos_and_settings().find(find_this)==infos_and_settings().end()) return {};
	double data = tools::str::str_to_double(infos_and_settings().at(find_this));
...
const quantity_t filecontents::dsims_t::contrast_aperture(const string find_this)
{
	if (infos_and_settings().find(find_this)==infos_and_settings().end()) return {};
	double data = tools::str::str_to_double(infos_and_settings().at(find_this));
...
const quantity_t filecontents::dsims_t::egate(const string find_this)
{
	if (infos_and_settings().find(find_this)==infos_and_settings().end()) return {};
	double data = tools::str::str_to_double(infos_and_settings().at(find_this));
...
const quantity_t filecontents::dsims_t::em_voltage(const string find_this)
{
	if (infos_and_settings().find(find_this)==infos_and_settings().end()) return {};
	double data = tools::str::str_to_double(infos_and_settings().at(find_this));
...
const quantity_t filecontents::dsims_t::em_yield(const string find_this)
{
	if (infos_and_settings().find(find_this)==infos_and_settings().end()) return {};
	double data = tools::str::str_to_double(infos_and_settings().at(find_this));
...
const quantity_t filecontents::dsims_t::entrance_slit(const string find_this)
{
	if (infos_and_settings().find(find_this)==infos_and_settings().end()) return {};
	double data = tools::str::str_to_double(infos_and_settings().at(find_this));
...
const quantity_t filecontents::dsims_t::energy_window(const string find_this)
{
	if (infos_and_settings().find(find_this)==infos_and_settings().end()) return {};
	double data = tools::str::str_to_double(infos_and_settings().at(find_this));
...
const quantity_t filecontents::dsims_t::exit_slit(const string find_this)
{
	if (infos_and_settings().find(find_this)==infos_and_settings().end()) return {};
	double data = tools::str::str_to_double(infos_and_settings().at(find_this));
...
const quantity_t filecontents::dsims_t::field_aperture(const string find_this)
{
	if (infos_and_settings().find(find_this)==infos_and_settings().end()) return {};
	double data = tools::str::str_to_double(infos_and_settings().at(find_this));
...
const quantity_t filecontents::dsims_t::mass_resolution(const string find_this)
{
	if (infos_and_settings().find(find_this)==infos_and_settings().end()) return {};
	double data = tools::str::str_to_double(infos_and_settings().at(find_this));
