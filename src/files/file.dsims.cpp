...
/***************************************/
/*****  files::dsims_t::column_t    ****/
/***************************************/

void files::dsims_t::column_t::to_screen()
{
...
// const sputter_depth_t files::dsims_t::Ipr_t::sputter_depth() const
// {
// 	return sputter_depth_p;
// }
// 
// const sputter_time_t files::dsims_t::Ipr_t::sputter_time() const
// {
// 	return sputter_time_p;
// }
// 
// const sputter_current_t files::dsims_t::Ipr_t::sputter_current() const
// {
// 	return sputter_current_p;
// }

files::dsims_t::Ipr_t::Ipr_t(sputter_current_t sputter_current_s, 
...
const std::__cxx11::string files::dsims_t::Ipr_t::to_string(const string del) const
{
	stringstream out;
...
files::dsims_t::dsims_t(filenames::dsims_t& name) : sims_t(name,"\t",{"*** DATA FILES ***"}), name(name)
{
}

void files::dsims_t::to_screen(string prefix)
{
...
// 		cluster.concentration_p=concentration_s;
		if (cluster.is_set()) collected_clusters.push_back(cluster);
			...
			logger::error("files::dsims_t::clusters() cluster.is_set()", false);
	}
	return collected_clusters;
}

const vector<files::dsims_t::column_t> files::dsims_t::columns()
{
	if (check_Ipr()) ipr_shift_correction();
	remove_corrupted_lines();
	if (cluster_names().size()<1 && !parse_units_dimensions_clusternames())
	{
		logger::debug("files::dsims_t::columns(): !parse_units_dimensions_clusternames");
		return {};
	}
...
const files::dsims_t::Ipr_t files::dsims_t::Ipr()
{
...
const vector<std::__cxx11::string>& files::dsims_t::dimensions()
{
	if (dimensions_p.size()>0) return dimensions_p;
	parse_units_dimensions_clusternames();
	return dimensions_p;
}
const vector<std::__cxx11::string>& files::dsims_t::units()
{
	if (units_p.size()>0) return units_p;
	parse_units_dimensions_clusternames();
	return units_p;
}
const vector<std::__cxx11::string>& files::dsims_t::cluster_names()
{
...
bool files::dsims_t::parse_units_dimensions_clusternames()
{
	...
	{
// 		error=true;
		logger::debug("files::dsims_t::parse_raw_header_tensor(): samples.size()!=1");
		return false;
	}
	...
	if (units_p.size()!=dimensions_p.size()) {
		logger::fatal("files::dsims_t::parse_raw_header_tensor(): units.size()!=dimensions.size()");
		return false;
	}
...
set<int> files::dsims_t::corrupted_lines()
{
...
void files::dsims_t::remove_corrupted_lines()
{
	...
	{
		logger::info("files::dsims_t::remove_corrupted_lines()", *it);
 ...
 */
bool files::dsims_t::ipr_shift_correction() 
{
	raw_data_tensor();
	vector<vector<string>>* mat = &raw_data_tensor_p.at(0);
    if (mat->size()==0) 
	{
		logger::error("files::dsims_t::ipr_shift_correction(): raw_data_tensor_p.at(0).size()==0", (int)mat->size());
		return false;
	}
...
const quantity_t files::dsims_t::get_quantity_from_dimension_and_clustername(const string col_dimension, const string ignore_clustername)
{
	string unit="";
	...
	if (unit=="") 
	{
		logger::error("files::dsims_t::sputter_time() unit",unit);
		return {};
	}
	return quantity_t{statistics::common_vector(data_vecs),unit};
}

const map<string,string>& files::dsims_t::infos_and_settings()
{
...
const tm files::dsims_t::start_date_time()
{
...
const tm files::dsims_t::creation_date_time()
{
...
const string files::dsims_t::secondary_polarity()
{
	return infos_and_settings().at("Secondary ion polarity");
}

const energy_t files::dsims_t::sputter_energy()
{
...
const quantity_t files::dsims_t::contrast_aperture()
{
