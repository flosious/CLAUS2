...
class Gnuplot
{
	...
	// ---------------------------------------------------
        void           init(); 
	// ---------------------------------------------------
	///\brief removes oldes temporary file   
	/// 
	/// \param --> none
	/// 
	/// \return <-- nothing
	// ---------------------------------------------------                          
        void    remove_oldest_tmpfile();  
...
vector<string> Gnuplot::get_tmpfiles() {
    return tmpfile_list;
}

void Gnuplot::remove_tmpfiles(){
    cout << "gnuplot remove tmpfiles called\n";
    if ((tmpfile_list).size() > 0)
    {
        for (unsigned int i = 0; i < tmpfile_list.size(); i++) {
            cout << "\tremoving: "<< tmpfile_list[i].c_str() << "\n";
...
/*
 * from https://github.com/orbitcowboy/gnuplot-cpp/issues/5
 */
