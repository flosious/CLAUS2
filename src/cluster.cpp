...
intensity_t& cluster_t::intensity()
{
	...
	if (concentration_p.is_set() && SF.is_set())
		intensity_p = concentration_p / SF;
	return intensity_p;
}
...
// 	new_cluster.sputter_depth_p = new_sputter_depth;
	if (sputter_depth_p.is_set())
		new_cluster.sputter_depth_p = sputter_depth_p.interp(old_Q,new_Q);
	
	if (sputter_time_p.is_set())
		new_cluster.sputter_time_p = sputter_time_p.interp(old_Q,new_Q);
	
	if (concentration_p.is_set())
		new_cluster.concentration_p = concentration_p.interp(old_Q,new_Q);
	
	if (intensity_p.is_set())
		new_cluster.intensity() = intensity_p.interp(old_Q,new_Q);
